...
const generateFakeOffers = (): Offer[] => [
  {
    ...
    rate: '6.875%',
    apr: '7.125%',
    monthlyPayment: '$4,478',
    loanType: '30-year Fixed',
    closingCosts: '$8,500',
  },
  {
    id: 'offer-2',
    lenderName: 'Metro Mortgage Corp',
    nmls: '#2156',
    rate: '6.950%',
    apr: '7.200%',
    monthlyPayment: '$4,526',
    loanType: '30-year Fixed',
    closingCosts: '$7,800',
  },
  {
...
const generateAdditionalOffers = (): Offer[] => [
  {
    id: 'offer-4',
    lenderName: 'Horizon Home Lending',
    nmls: '#4521',
    rate: '7.050%',
    apr: '7.275%',
    monthlyPayment: '$4,592',
    loanType: '30-year Fixed',
    closingCosts: '$8,750',
  },
  {
    id: 'offer-5',
    lenderName: 'Liberty Financial Group',
    nmls: '#5892',
    rate: '7.100%',
    apr: '7.325%',
    monthlyPayment: '$4,625',
    loanType: '30-year Fixed',
    closingCosts: '$8,300',
  },
  {
    id: 'offer-6',
    lenderName: 'Summit Lending Partners',
    nmls: '#6734',
    rate: '7.125%',
    apr: '7.350%',
    monthlyPayment: '$4,641',
    loanType: '30-year Fixed',
    closingCosts: '$7,950',
  },
  {
    id: 'offer-7',
    lenderName: 'Nationwide Trust Bank',
    nmls: '#7208',
    rate: '7.150%',
    apr: '7.375%',
    monthlyPayment: '$4,658',
    loanType: '30-year Fixed',
    closingCosts: '$8,600',
  },
]
...
export const usePrototypeStore = create<PrototypeStore>((set, get) => ({
  ...
  setCurrentTab: (tab) => {
    ...
    // If navigating to private marketplace for the first time, show entry modal and prefill James data
      ...
      if (!state.privateMarketplace.hasEnteredMarketplace) {
        set({
          privateMarketplace: {
            ...
            filters: {
              creditScore: '', // Will be set in entry modal
              zipCode: '10011',
          ...
          leads: [
            {
              ...
              formSnapshot: {
                ...
                loanAmount: '$680,000',
                downPayment: '20',
                creditScore: '', // BLANK - will be filled in Private Marketplace
                loanTerm: '30 year fixed',
                militaryStatus: 'Non-Military',
                dtiRatio: 'lt40',
                mortgagePoints: 'All',
  ...
  resetCurrentTab: () => {
    ...
    if (currentTab === 'agency-admin') {
      set({ agencyAdminForm: initialAgencyAdminForm })
    } else if (currentTab === 'agent-signup') {
      set({ agentSignupForm: initialAgentSignupForm })
    } else if (currentTab === 'agent-dashboard') {
      set({ agentDashboard: initialAgentDashboard })
    } else if (currentTab === 'private-marketplace') {
      set({ privateMarketplace: initialPrivateMarketplace })
    }
  },
    ...
    set({
      agentDashboard: {
        ...
        createLinkModal: {
          ...
          formFields: {
            ...get().agentDashboard.createLinkModal.formFields,
            [field]: value,
          },
        },
      },
  ...
  generateLink: () => {
    ...
    set({
      agentDashboard: {
        ...state.agentDashboard,
        createLinkModal: {
          ...state.agentDashboard.createLinkModal,
          isLoading: true,
        },
      },
    })
    ...
    setTimeout(() => {
      ...
      const newLead: Lead = {
        ...
        formSnapshot: {
          ...
          dtiRatio: fields.dtiRatio,
          mortgagePoints: fields.mortgagePoints,
          propertyType: fields.propertyType,
          propertyUse: fields.propertyUse,
          includeFHA: fields.includeFHA,
        },
      }
  ...
  regenerateLink: () => {
    ...
    set({
      agentDashboard: {
        ...state.agentDashboard,
        regenerateModal: {
          ...state.agentDashboard.regenerateModal,
          isLoading: true,
        },
      },
    })
      ...
      set({
        agentDashboard: {
          ...
          leads: get().agentDashboard.leads.map(lead =>
            lead.id === leadId
              ? { ...lead, linkUrl: newLink, sentAt: new Date() }
              : lead
          ),
          regenerateModal: {
            ...get().agentDashboard.regenerateModal,
            isLoading: false,
            newLink,
          },
        },
  ...
  updateLeadStatus: (email, status) => {
    set({
      agentDashboard: {
        ...get().agentDashboard,
        leads: get().agentDashboard.leads.map(lead =>
          lead.email === email ? { ...lead, status } : lead
        ),
      },
    })
  },
  ...
  openEntryModal: () => {
    set({
      privateMarketplace: {
        ...
        entryModal: {
          ...get().privateMarketplace.entryModal,
          isOpen: true,
        },
      },
    })
  ...
  findOffers: () => {
    ...
    // Close entry modal and set loading
    set({
      privateMarketplace: {
        ...state.privateMarketplace,
        entryModal: {
          ...state.privateMarketplace.entryModal,
          isOpen: false,
        },
        filters: {
          ...state.privateMarketplace.filters,
          creditScore,
        },
  ...
  updateFilter: (field, value) => {
    set({
      privateMarketplace: {
        ...get().privateMarketplace,
        filters: {
          ...get().privateMarketplace.filters,
          [field]: value,
        },
      },
    })
  ...
  toggleShowMoreOffers: () => {
    set({
      privateMarketplace: {
        ...get().privateMarketplace,
        showMoreOffers: !get().privateMarketplace.showMoreOffers,
      },
    })
  },

  openGetQuoteModal: (lenderId) => {
    const buyerEmail = get().privateMarketplace.buyerIdentity.email
  ...
  toggleLenderSelection: (lenderId) => {
    ...
    if (isSelected) {
      newSelection = current.filter(id => id !== lenderId)
    } else {
      newSelection = [...current, lenderId]
    }
